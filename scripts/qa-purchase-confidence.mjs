...
const checks = [
  ...
  ["saved room decision summary", files.saved.includes('id="saved-room-decision-summary"')],
  ["saved room per-piece evidence", files.module.includes("saved-room-item__evidence")],
  ["room brief consultation handoff", files.module.includes("function enhanceConsultationHandoff")],
  ["room constraints carried to consultation", files.module.includes('params.set(key, String(value))')],
  ["compare browser truth label", files.compare.includes("Prototype state · this browser only")],
  ["compare room-fit evidence", files.module.includes('["Room fit"')],
  ...
  ["compare returnability evidence", files.module.includes('["Returns"')],
